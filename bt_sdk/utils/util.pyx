...
cdef UuidFrame fast_uuid4_c() nogil: # C数组 ---> python object, 视为以\0结尾的字符串类似于C++ string
    cdef UuidFrame uu
...
cdef object num2date(double x, bint native=True, bint ordinal=True):
    """
    Unix 时间戳直接算术转换
    """
...
cdef MarketTime market_utc(int64_t ts, bint native=True) nogil :
    cdef MarketTime mt
...
cdef int64_t ts_to_int_date(int64_t ts, bint native=True) nogil: # only cdef nogil
