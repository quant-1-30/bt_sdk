...
def int2dt(ts):
    ...
    dt = dt.astimezone(tz=pytz.timezone('Asia/Shanghai'))
    return dt


def gt_dt(dt, other_dt, fmt='%Y%m%d%H%M%S'):
...
# def offset2dt(dt, offset, fmt="%Y%m%d%H%M%S"):
#     struct_dt = datetime.datetime.strptime(dt, fmt)
#     offset_dt = struct_dt + offset
#     to_str = offset_dt.strftime(fmt)
#     return to_str

# def dt2freq(dt, freq):
#     _dt = num2date(dt)
#     date_range = pd.date_range(start=_dt, end=_dt, freq=freq)
#     return [date2num(dt) for dt in date_range]

# def elapsed(dt, fmt="%Y%m%d%H%M%S") :
#     # %-m 不补0
#     fmt = str2dt(dt, fmt)
#     if fmt.hour <=12:
#         delta = fmt - datetime.datetime(year=fmt.year, month=fmt.month, day=fmt.day, hour=9, minute=30)
#     else:
#         delta = fmt - datetime.datetime(year=fmt.year, month=fmt.month, day=fmt.day, hour=13, minute=0) + datetime.timedelta(hours=2)
...
def market_utc(date, tzinfo="Asia/Shanghai"):
    format_dt = ensure_utc(date, default=tzinfo)
...
def market_offset(loc, dts):
    """
        offset: 3s tick index
        trading hours: 4 --- 9:30 - 11:30 / 1:00 - 3:00 
    """
    isAm = loc - 2*60*60/3
    dt = datetime.datetime.strptime(dt, fmt="%Y%m%d")
    if isAm:
        offset_dt = dt + datetime.timedelta(hours=9, minutes=30) + datetime.timedelta(seconds=loc * 3) 
    else:
        offset_dt = dt + datetime.timedelta(hours=13) + datetime.timedelta(seconds=isAm * 3)
    return offset_dt
